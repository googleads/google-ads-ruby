# frozen_string_literal: true
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: google/ads/googleads/v14/resources/campaign_extension_setting.proto

require 'google/protobuf'

require 'google/ads/google_ads/v14/enums/extension_setting_device_pb'
require 'google/ads/google_ads/v14/enums/extension_type_pb'
require 'google/api/field_behavior_pb'
require 'google/api/resource_pb'


descriptor_data = "\nCgoogle/ads/googleads/v14/resources/campaign_extension_setting.proto\x12\"google.ads.googleads.v14.resources\x1a=google/ads/googleads/v14/enums/extension_setting_device.proto\x1a\x33google/ads/googleads/v14/enums/extension_type.proto\x1a\x1fgoogle/api/field_behavior.proto\x1a\x19google/api/resource.proto\"\xd7\x04\n\x18\x43\x61mpaignExtensionSetting\x12P\n\rresource_name\x18\x01 \x01(\tB9\xe0\x41\x05\xfa\x41\x33\n1googleads.googleapis.com/CampaignExtensionSetting\x12\\\n\x0e\x65xtension_type\x18\x02 \x01(\x0e\x32?.google.ads.googleads.v14.enums.ExtensionTypeEnum.ExtensionTypeB\x03\xe0\x41\x05\x12@\n\x08\x63\x61mpaign\x18\x06 \x01(\tB)\xe0\x41\x05\xfa\x41#\n!googleads.googleapis.com/CampaignH\x00\x88\x01\x01\x12M\n\x14\x65xtension_feed_items\x18\x07 \x03(\tB/\xfa\x41,\n*googleads.googleapis.com/ExtensionFeedItem\x12\x61\n\x06\x64\x65vice\x18\x05 \x01(\x0e\x32Q.google.ads.googleads.v14.enums.ExtensionSettingDeviceEnum.ExtensionSettingDevice:\x89\x01\xea\x41\x85\x01\n1googleads.googleapis.com/CampaignExtensionSetting\x12Pcustomers/{customer_id}/campaignExtensionSettings/{campaign_id}~{extension_type}B\x0b\n\t_campaignB\x8f\x02\n&com.google.ads.googleads.v14.resourcesB\x1d\x43\x61mpaignExtensionSettingProtoP\x01ZKgoogle.golang.org/genproto/googleapis/ads/googleads/v14/resources;resources\xa2\x02\x03GAA\xaa\x02\"Google.Ads.GoogleAds.V14.Resources\xca\x02\"Google\\Ads\\GoogleAds\\V14\\Resources\xea\x02&Google::Ads::GoogleAds::V14::Resourcesb\x06proto3"

pool = Google::Protobuf::DescriptorPool.generated_pool

begin
  pool.add_serialized_file(descriptor_data)
rescue TypeError => e
  # Compatibility code: will be removed in the next major version.
  require 'google/protobuf/descriptor_pb'
  parsed = Google::Protobuf::FileDescriptorProto.decode(descriptor_data)
  parsed.clear_dependency
  serialized = parsed.class.encode(parsed)
  file = pool.add_serialized_file(serialized)
  warn "Warning: Protobuf detected an import path issue while loading generated file #{__FILE__}"
  imports = [
  ]
  imports.each do |type_name, expected_filename|
    import_file = pool.lookup(type_name).file_descriptor
    if import_file.name != expected_filename
      warn "- #{file.name} imports #{expected_filename}, but that import was loaded as #{import_file.name}"
    end
  end
  warn "Each proto file must use a consistent fully-qualified name."
  warn "This will become an error in the next major version."
end

module Google
  module Ads
    module GoogleAds
      module V14
        module Resources
          CampaignExtensionSetting = ::Google::Protobuf::DescriptorPool.generated_pool.lookup("google.ads.googleads.v14.resources.CampaignExtensionSetting").msgclass
        end
      end
    end
  end
end
