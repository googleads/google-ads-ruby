...
def add_complete_campaigns_using_batch_job(customer_id)
  ...
  batch_job_service_client = client.service.batch_job

  batch_job_resource_name = create_batch_job(
    client,
    batch_job_service_client,
    customer_id,
  )
  ...
  fetch_and_print_results(batch_job_service_client, batch_job_resource_name)
end

# Creates a new batch job for the specified customer ID.
def create_batch_job(client, batch_job_service_client, customer_id)
  # Creates a batch job operation to create a new batch job.
  operation = client.operation.create_resource.batch_job do |bj|
  end

  # Issues a request to the API and get the batch job's resource name.
  response = batch_job_service_client.mutate_batch_job(
...
def add_all_batch_job_operations(
  ...
  customer_id,
  batch_job_resource_name)
  response = batch_job_service_client.add_batch_job_operations(
...
# operations.
def run_batch_job(batch_job_service_client, batch_job_resource_name)
  operation_response = batch_job_service_client.run_batch_job(
    resource_name: batch_job_resource_name,
  )
...
def fetch_and_print_results(batch_job_service_client, batch_job_resource_name)
  ...
  batch_job_results = batch_job_service_client.list_batch_job_results(
    resource_name: batch_job_resource_name,
    page_size: PAGE_SIZE,
  )
...
def build_all_operations(client, customer_id)
  ...
  mop = client.operation.mutate
  mop.campaign_budget_operation = campaign_budget_operation
  mutate_operations << mop
  ...
  campaign_criterion_operations.each do |op|
    mop = client.operation.mutate
    mop.campaign_criterion_operation = op
    mutate_operations << mop
  end
  ...
  ad_group_criterion_operations.each do |op|
    mop = client.operation.mutate
    mop.ad_group_criterion_operation = op
    mutate_operations << mop
  end
...
# MutateJobService returns information about such errors.
