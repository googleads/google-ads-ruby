...
def get_invoices(customer_id, billing_setup_id)
  ...
  response.invoices.each do |invoice|
    puts "- Found the invoice '#{invoice.resource_name}'" \
      ...
      " Adjustments: subtotal '#{micro_to_base(invoice.adjustments_subtotal_amount_micros)}'," \
      " tax '#{micro_to_base(invoice.adjustments_tax_amount_micros)}'," \
      " total '#{micro_to_base(invoice.adjustments_total_amount_micros)}'\n"  \
      " Regulatory costs: subtotal '#{micro_to_base(invoice.regulatory_costs_subtotal_amount_micros)}'," \
      ...
      " Amounts: subtotal '#{micro_to_base(invoice.subtotal_amount_micros)}'," \
    ...
    invoice.account_budget_summaries.each do |account_budget_summary|
      puts "  - Account budget '#{account_budget_summary.account_budget}':\n" \
        ...
        "      Amounts: subtotal '#{micro_to_base(account_budget_summary.subtotal_amount_micros)}'," \
